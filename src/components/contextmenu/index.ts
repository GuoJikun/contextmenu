...
import MouseMenu from "./src/ContextMenu.vue";
  ...
  if (value.options.length > 0) {
    contextMenuEvent = (e: MouseEvent) => {
      if (typeof value.disabled === "function" && value.disabled(value.params))
        return;
      e.preventDefault();
      const MouseMenuCtx = ContextmenuFunction({
        el,
        ...value,
      });
      const { x, y } = e;
    ...
    if (value.useLongPressInMobile && "ontouchstart" in window) {
      longPressEvent = (e: TouchEvent) => {
        if (
          ...
          value.disabled(value.params)
        )
          return;
        e.preventDefault();
        MouseMenuCtx = ContextmenuFunction({
          el,
          ...value,
        });
        const { touches } = e;
...
MouseMenu.install = (app: App, type: string = "component"): void => {
  if (type === "component") {
    app.component("yak-contextmenu", MouseMenu);
  } else if (type === "directive") {
    app.directive("contextmenu", ContextmenuDirective);
  } else if (type === "function") {
    app.config.globalProperties.$contextmenu = ContextmenuFunction;
  } else {
    app.component("yak-contextmenu", MouseMenu);
    app.directive("contextmenu", ContextmenuDirective);
    app.config.globalProperties.$contextmenu = ContextmenuFunction;
